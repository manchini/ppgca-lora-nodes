  ...
  const lmic_pinmap lmic_pins = {
    ...
    // mq7
    #define mq7pin 1//the AOUT pin of the CO sensor goes into analog pin A0 of the arduino
    ...
    void loop() {
      ...
      if(!radioON){
        if(currentMillis<5000){ // avg pressao
          ...
          mpxValue = mpxValue/50;

          if(mpxValue>mpxAverage +3
            //&& mpxAverage-(int)mpxAverage!=0.0
          ){//gamb de noiser
              car = !car;
              if(car){
                countTraffic++;
